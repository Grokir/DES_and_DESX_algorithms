...
void DES::setBinaryKEY(const std::string& binkey) {
  this->clear_bin_str(this->binkey);
...
void DES::setBinaryMSG(const std::string& binmsg) {
  this->clear_bin_str(this->binmsg);
...
const uint DES::countPlainTextBits() const {
  return size_of_block;
};

const uint DES::countPlainTextSymbols() const {
  return ( size_of_block / size_of_char );
};

const uint DES::countKeyBits() const {
  return size_of_key;
};
...
void DES::clear_bin_str(std::vector<uint> &v){
  for(std::size_t i = 0; i < v.size(); i++)
    v[i] = 0;
};

uint DES::char_to_binint(char ch){
  if( ch == '0' ) return 0;
  return 1;
};

char DES::binint_to_char(int i){
  return static_cast<char>(i + 48);
};

void DES::left_shift_key(std::vector<uint>& key){
...
void DES::right_shift_key(std::vector<uint>& key){
  std::size_t  n   = key.size();
  uint         tmp = key[n - 1];
...
std::vector<std::vector<uint>> DES::preparing_keys(const std::vector<uint> &key) {
...
std::vector<uint> DES::IP(const std::vector<uint> &msg) {
  /*
...
std::vector<uint> DES::inverse_IP(const std::vector<uint> &msg) {
  /*
    Теоретическая перестановка:
...
std::vector<uint> DES::perm_with_expansion(const std::vector<uint> &v) {
  /* 
...
std::vector<uint> DES::KEY_perm_with_choice(const std::vector<uint> &v) {
...
std::vector<uint> DES::KEY_perm_with_choice2(const std::vector<uint> &v) {
  /* 
    Теоретическая перестановка с выбором 1 для ключа:
      14 17 11 24  1  5
       3 28 15  6 21 10
...
std::vector<uint> DES::one_encrypt_round(const std::vector<uint>& key) {
...
void DES::one_decrypt_round(const std::vector<uint> &key) {
  std::vector<uint> L       ( size_of_block / 2 );
  std::vector<uint> R       ( size_of_block / 2 );
...
std::vector<uint> DES::F(const std::vector<uint> &msgR, const std::vector<uint> &key) {
  return perm_P(
           perm_with_choice(
             XOR(
...
std::vector<uint> DES::XOR(const std::vector<uint> &lhs, const std::vector<uint> &rhs) {
...
std::vector<uint>/* 32 bits */ DES::perm_with_choice(/* 48 bits */const std::vector<uint> &v) {
  ...
  /* https://ru.wikipedia.org/wiki/S-%D0%B1%D0%BB%D0%BE%D0%BA_(%D0%B8%D0%BD%D1%84%D0%BE%D1%80%D0%BC%D0%B0%D1%82%D0%B8%D0%BA%D0%B0)#/media/%D0%A4%D0%B0%D0%B9%D0%BB:DES_S-box.jpg */
...
std::vector<uint> DES::perm_P(const std::vector<uint> &v) {
  /*
